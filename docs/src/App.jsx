            ...
            {selectedFunction ? (
              <div className="max-w-3xl mx-auto">
                <div className="mb-6 pb-4 border-b border-border">
                  <h1 className="text-xl md:text-2xl font-bold mb-2">{selectedFunction.name}</h1>
                  <p className="text-sm md:text-base text-muted-foreground">{selectedFunction.description}</p>
                </div>
                  ...
                  {selectedFunction.parameters.length > 0 ? (
                    <ul className="space-y-2 md:space-y-3">
                      {selectedFunction.parameters.map((param, index) => (
                        <li key={index} className="flex flex-col hover-scale p-2 md:p-3 rounded-md border border-border bg-background transition-colors">
                          <div className="flex items-center flex-wrap gap-2">
                            <span className="font-medium text-foreground">{param.name}</span>
                            <code className="text-xs md:text-sm bg-secondary px-1.5 md:px-2 py-0.5 rounded font-mono">{param.type}</code>
                            {param.default && (
                          ...
                          <span className="text-xs md:text-sm text-muted-foreground mt-1">{param.description}</span>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-muted-foreground italic text-sm">No parameters</p>
                  )}
                </div>
                ...
                <div className="mb-6 md:mb-8">
                  ...
                  <code className="text-xs md:text-sm bg-secondary px-2 md:px-3 py-1.5 md:py-2 rounded-md font-mono">{selectedFunction.returnType}</code>
                </div>
              ...
              <div className="max-w-3xl mx-auto text-center">
                <h1 className="text-2xl md:text-4xl font-bold mb-3 md:mb-4 bg-clip-text text-transparent bg-gradient-to-b from-foreground to-muted-foreground">MathStuff Documentation</h1>
                ...
                <div className="text-left p-4 md:p-6 border border-border rounded-lg bg-card/50 shadow-sm">
                  <h2 className="text-lg md:text-xl font-semibold mb-4 md:mb-6">Getting Started</h2>
                  <p className="mb-3 md:mb-4 text-sm md:text-base">Add the JitPack repository to your <code className="text-xs md:text-sm bg-muted px-1.5 md:px-2 py-0.5 rounded font-mono">settings.gradle.kts</code> file:</p>
                  <CodeBlock
                  ...
                  <CodeBlock
                    code={`implementation("com.github.zahid4kh:mathstuff:1.0.9")`}
                    language="kotlin"
                  />
...
`}
                ...
                <p className="mt-6 md:mt-8 text-sm md:text-base text-muted-foreground flex items-center justify-center gap-2">
                  ...
                  <span className="hidden md:inline">Select a function from the sidebar to view its documentation</span>
                </p>
              </div>
            )}
          </div>
